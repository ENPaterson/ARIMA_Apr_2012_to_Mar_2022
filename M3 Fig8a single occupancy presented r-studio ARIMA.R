...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=single_hh_presented_n))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=single_hh_presented_n))+
  geom_line()+
  labs(
    y = "Presented (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=single_hh_presented_n))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
Apr2012_Feb2022 <- ARIMA$single_hh_presented_n[1:95]      

PANDEMIC2020_2022 <- ARIMA$single_hh_presented_n[96:120]   
...
ggplot(data = forecast_arima_Apr2012_Feb2022_df,
  ...
  ylim(0, 1500)
ggsave("M3_8a.pdf", width = 15, height = 15)
...
ggplot(data = forecast_arima_Apr2012_Feb2022_df,
  ...
  theme(legend.position = "none") +  
  ylim(0, 500)
ggsave("M3_8a zoom.pdf", width = 15, height = 15)