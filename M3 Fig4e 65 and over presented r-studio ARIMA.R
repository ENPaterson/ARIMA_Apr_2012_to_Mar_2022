...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=presented_n_65))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=presented_n_65))+
  geom_line()+
  labs(
    y = "Presented (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=presented_n_65))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
Apr2012_Feb2022 <- ARIMA$presented_n_65[1:95]      

PANDEMIC2020_2022 <- ARIMA$presented_n_65[96:120]   
...
ggplot(data = forecast_arima_Apr2012_Feb2022_df,
  ...
  ylim(0, 750)
ggsave("M3_4e.pdf", width = 15, height = 15)
...
ggplot(data = forecast_arima_Apr2012_Feb2022_df,
  ...
  theme(legend.position = "none") +  
  ylim(0, 100)
ggsave("M3_4e zoom.pdf", width = 15, height = 15)