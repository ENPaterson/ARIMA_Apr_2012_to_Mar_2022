...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=SI_presented_n))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=SI_presented_n))+
  geom_line()+
  labs(
    y = "Presented (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=SI_presented_n))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
Apr2012_Feb2022 <- ARIMA$SI_presented_n[1:95]      

PANDEMIC2020_2022 <- ARIMA$SI_presented_n[96:120]   
...
ggplot(data = forecast_arima_Apr2012_Feb2022_df,
  ...
  ylim(0, 1250)
ggsave("M3_2c.pdf", width = 15, height = 15)
